defmodule LiveStyle.Compiler.CSS.Vars do
  @moduledoc """
  ...
  def generate_properties(manifest) do
    ...
    |> Enum.map_join("\n", fn {_key, entry} ->
      ...
      "@property #{ident} { syntax: \"#{syntax}\"; inherits: #{inherits}; initial-value: #{initial_value} }"
    end)
  end
  ...
  defp flatten_var_entry(ident, key, val, at_rules)
    ...
    [{at_rules, ident, to_string(val)}]
  end

  # Handle default key with nested map
  defp flatten_var_entry(ident, key, val, at_rules) when key in [:default, "default"] do
    flatten_var_value(ident, val, at_rules)
  end

  # Handle at-rule conditions
  defp flatten_var_entry(ident, key, val, at_rules) do
    key_str = to_string(key)
    flatten_var_value(ident, val, at_rules ++ [key_str])
  end
