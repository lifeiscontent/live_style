defmodule LiveStyle.Value do
  ...
  alias LiveStyle.Value.Identifier
  alias LiveStyle.Value.Normalize
  alias LiveStyle.Value.Number
  alias LiveStyle.Value.Quote
  ...
  def to_css(v, "transition-property") when is_binary(v),
    do: v |> Normalize.normalize() |> Identifier.convert_snake_case_to_dash_case()
  ...
  def to_css(v, "transition-property") when is_atom(v),
    do: v |> Atom.to_string() |> Identifier.convert_snake_case_to_dash_case()
  ...
  # This allows:
  #
  #   css_class :parent, %{css_var({Tokens, :vars, :childColor}) => "red"}
  #
  # to generate:
  #
  #   .x123{--vabc:red}
