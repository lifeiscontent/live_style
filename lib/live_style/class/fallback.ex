defmodule LiveStyle.Class.Fallback do
  @moduledoc false
  ...
  @doc false
  @spec transform_array(String.t(), list()) :: {String.t(), keyword()}
  def transform_array(css_prop, values) do
    ...
    build_result(css_prop, class_name, transformed)
  end

  @doc false
  @spec transform_fallback(String.t(), list()) :: {String.t(), keyword()}
  def transform_fallback(css_prop, values) do
    normalized_values =
      values
      |> Enum.map(&CSSValue.to_css(&1, css_prop))
  ...
  @spec build_result(String.t(), String.t(), list()) :: {String.t(), keyword()}
  defp build_result(css_prop, class_name, [single]) do
    # Single value after transformation - use regular Builder
    result = Builder.create_entry(css_prop, single)
    # Override class name to use fallback hash
    result = Keyword.put(result, :class, class_name)
    {css_prop, result}
  end

  defp build_result(css_prop, class_name, multiple) do
    # Multiple fallback values - use Builder.create_entry_with_fallbacks
    result = Builder.create_entry_with_fallbacks(css_prop, class_name, multiple)
    {css_prop, result}
  end
